...
def decode(line, recursive):
    ...
    while len(text):
        ...
        text = text[pos+1:]
        pos = text.find("(")
        params = map(lambda x: int(x), tag.split("x"))
        content = text[:params[0]]
        if recursive:
            size += decode(content, recursive) * params[1]
        else:
...
def main_1(input_stream):
    ...
    for line in input_stream:
        size += decode(line, False)
    print size    

def main_2(input_stream):
    size = 0
    for line in input_stream:
        size += decode(line, True)
    print size    
